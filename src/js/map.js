...
import { data } from "../constants/fakeGeoJson.js";

console.log(data);

function riskColor(feature) {
  const color = feature.properties.risk["day1"] > 1 ? "red" : "blue";
  return {
    fillColor: color,
    fillOpacity: 0.6,
    strokeWeight: 2,
...
export default function drawMap() {
  ...
  // axios.get("http://mathew.calsurv.org/zika/layer").then(res => {
  //   let cachedData = res.data;
  //   for (var i = 0; i < cachedData.features.length; i++) {
  //     var g = decodeGeometry(cachedData.features[i].geometry);
  //     cachedData.features[i].geometry = g;
  //   }
  //   console.log(cachedData);
  //   // map.data.addGeoJson(cachedData);
  //   // map.data.setStyle(colorize);
  // });
  ...
  // console.log(decodedFresno);

  // const setPath = new google.maps.Polygon({
  //   path: decodedFresno,
  //   strokeColor: "red",
  //   strokeOpacity: 1,
  //   strokeWeight: 4,
