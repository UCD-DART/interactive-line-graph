...
export const Chart = function(divId, riskObj, divWidth, initDate) {
  ...
  function brushed() {
    if (document.readyState == 'complete' && event.selection[0] > 5 && event.selection[1] < width) {
    ...
    svg
      .select('.zoom')
      .call(zoom.transform, zoomIdentity.scale(width / (s[1] - s[0])).translate(-s[0], 0));
  }
  ...
  function drawGraph() {
    ...
    const initialMarkerPlace = x2(riskObj[document.querySelector('#pickDate').value].date);
